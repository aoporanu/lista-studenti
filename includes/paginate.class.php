 ...
 *
 * @author    Aaron D. Campbell <AaronCampbell@ezdispatch.com>
 *
 * @version 1.0.3
 *
 * Changes:
...
class Paginate {
     ...
     * @param string $page_var - $_GET index to use for "Current Page"
     *
     * @return void
     */
    ...
    public function check_page_var() {
        ...
        */
        if (!isset($_GET[$this->page_var]) || (int)$_GET[$this->page_var] <= 0) {
            ...
            $_GET[$this->page_var] = (int)$_GET[$this->page_var];
        }
        if ($this->per_page != 0) {
            $this->create_query();
        }
    ...
    public function show_result_text() {
        $start = (($_GET[$this->page_var]-1) * $this->per_page)+1;
    ...
    public function show_pages() {
        ...
        */
        ...
        if ($this->total_pages <= $this->max_show_all || $this->max_show_all == 0) {
            ...
            */
            if($_GET[$this->page_var] >= $this->links_each_side+2) {
                $page_array[] = $this->get_page_link(1);
            }
            // If needed, add an ellipsis after page one.
            if($_GET[$this->page_var] >= $this->links_each_side+3) {
                $page_array[] = '...';
            }
            //Set the first page to be added (for pages in the main group)
            if ($_GET[$this->page_var]-$this->links_each_side <= 1) {
                $start = 1;
            } elseif ($_GET[$this->page_var] > $this->total_pages-$this->links_each_side) {
                $start = $this->total_pages-(2*$this->links_each_side);
            } else {
                $start = $_GET[$this->page_var]-$this->links_each_side;
            }
            //Set the last page to be added (for pages in the main group)
            if ($_GET[$this->page_var]+$this->links_each_side >= $this->total_pages) {
                $end = $this->total_pages;
            } elseif ($_GET[$this->page_var] < $this->links_each_side+1) {
                $end = (2*$this->links_each_side)+1;
            } else {
                $end = $_GET[$this->page_var]+$this->links_each_side;
            }
            ...
            if($_GET[$this->page_var] <= $this->total_pages-$this->links_each_side-2) {
                $page_array[] = '...';
            }
            /*
            ...
            */
            if($_GET[$this->page_var] <= $this->total_pages-$this->links_each_side-1) {
     ...
     * $_GET[$this->page_var].  It also returns only text (no link) if $p is the current
     * page.
     *
    ...
    private function get_page_link($p, $text=NULL) {
        ...
        if ($p != $_GET[$this->page_var]) {
            $cur_page = $_GET[$this->page_var];
            $_GET[$this->page_var] = $p;
            $get_string = array();
            foreach ($_GET as $k=>$v) {
                $get_string[] = "{$k}={$v}";
            }
            $_GET[$this->page_var] = $cur_page;
    ...
    private function create_query() {
        ...
        $start = ($_GET[$this->page_var]-1) * $this->per_page;
